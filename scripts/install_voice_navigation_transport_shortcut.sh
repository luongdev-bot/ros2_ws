#!/usr/bin/env bash
# Put a "Navigation Transport (Voice, Vietnamese)" launcher on the Desktop and in the app menu.
#
# Usage:  ./install_voice_navigation_transport_shortcut.sh [workspace_dir]

set -euo pipefail

WORKSPACE="${1:-${ARM_MOTION_WS:-$HOME/ros2_ws}}"
LAUNCHER="${WORKSPACE}/scripts/run_voice_navigation_transport.sh"
...
ENTRY="${APPS_DIR}/voice-navigation-transport.desktop"
cat >"${ENTRY}" <<DESKTOP
[Desktop Entry]
Type=Application
Version=1.0
Name=Navigation Transport (Voice, Vietnamese)
Comment=Navigate the mapped warehouse by named destinations using Vietnamese voice commands
Exec=${LAUNCHER}
Path=${WORKSPACE}
Terminal=false
...
DESKTOP_COPY="${DESKTOP_DIR}/voice-navigation-transport.desktop"
