#!/usr/bin/env bash
# Put a "Transport & Delivery (Voice, Vietnamese)" launcher on the Desktop and in the app menu.
#
# Usage:  ./install_voice_transport_dietitianl_shortcut.sh [workspace_dir]

set -euo pipefail

WORKSPACE="${1:-${ARM_MOTION_WS:-$HOME/ros2_ws}}"
LAUNCHER="${WORKSPACE}/scripts/run_voice_transport_dietitianl.sh"
...
ENTRY="${APPS_DIR}/voice-transport-dietitianl.desktop"
cat >"${ENTRY}" <<DESKTOP
[Desktop Entry]
Type=Application
Version=1.0
Name=Transport & Delivery (Voice, Vietnamese)
Comment=Grasp colour objects and deliver them to named warehouse destinations using Vietnamese voice commands
Exec=${LAUNCHER}
Path=${WORKSPACE}
Terminal=false
...
DESKTOP_COPY="${DESKTOP_DIR}/voice-transport-dietitianl.desktop"
