#!/usr/bin/env bash
# Launch the voice-controlled colour-sorting demo. The grasp demo provides
# Gazebo, colour perception, and the grasp executor; the voice agent listens
# continuously for Vietnamese instructions.
# Usage:  run_voice_color_sort.sh
#
#   Terminal 1: Colour sort (Gazebo + grasp executor, automatic grasp disabled)
#   Terminal 2: Voice agent (Vietnamese microphone input and Piper speech)
set -u
...
echo "Đã mở 2 terminal: Colour sort (Gazebo + grasp) và Voice agent tiếng Việt."
echo
echo "Không cần nhập lệnh: voice agent tự ghi âm liên tục từ micro theo từng đoạn"
echo "record_seconds giây (mặc định 5 giây), xử lý yêu cầu rồi phát câu trả lời qua loa."
echo
