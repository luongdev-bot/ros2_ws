#!/usr/bin/env bash
# Put a "Voice Agent (chọn kịch bản)" launcher on the Desktop and in the app menu.
#
# Usage:  ./install_voice_agent_menu_shortcut.sh [workspace_dir]

set -euo pipefail

WORKSPACE="${1:-${ARM_MOTION_WS:-$HOME/ros2_ws}}"
LAUNCHER="${WORKSPACE}/scripts/run_voice_agent.sh"
...
ENTRY="${APPS_DIR}/voice-agent-menu.desktop"
cat >"${ENTRY}" <<DESKTOP
[Desktop Entry]
Type=Application
Version=1.0
Name=Voice Agent (chọn kịch bản)
Comment=Choose a Vietnamese voice-agent scenario and its Gazebo world before startup
Exec=${LAUNCHER}
Path=${WORKSPACE}
Terminal=false
...
DESKTOP_COPY="${DESKTOP_DIR}/voice-agent-menu.desktop"
