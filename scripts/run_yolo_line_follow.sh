#!/usr/bin/env bash
# Launch the YOLO-segmentation variant of the figure-8 line-following demo:
# the simulation and arm setup are the same, but line detection uses a trained
# YOLO11n-seg model instead of the LAB-threshold detector.
# Usage:  run_yolo_line_follow.sh
#
#   Terminal 1: Gazebo (line-follow figure-8) + controllers
#   Terminal 2: arm_motion_server  (serves PlayMotion from the .d6a library)
#   Terminal 3: yolo_line_follow   (waits for /clock, then plays line_follow_init
#                                   before driving the base on success)
#   Terminal 4: Camera view        (rqt_image_view -> /yolo_line_follow/debug_image)
#
...
die() {
  ...
  if command -v zenity >/dev/null 2>&1; then
    zenity --error --title="Line follow YOLO" --text="$msg" 2>/dev/null
  elif command -v notify-send >/dev/null 2>&1; then
    notify-send "Line follow YOLO" "$msg" 2>/dev/null
...
MODEL="$WS/install/line_follow/share/line_follow/models/line_yolo11n_seg.pt"
if [ ! -f "$MODEL" ]; then
  die "YOLO line-follow model not installed: $MODEL is missing. Run: cd $WS && colcon build --packages-select line_follow"
fi

YOLO_PYTHON="$HOME/ultralytics_env/bin/python3"
if [ ! -f "$YOLO_PYTHON" ]; then
  die "YOLO Python virtual environment is missing: $YOLO_PYTHON. yolo_line_follow.launch.py requires the separate Python venv with torch+ultralytics; its default location is $HOME/ultralytics_env."
fi
...
if [ "$already_up" = "yes" ]; then
  ...
  open_term "Line follow (YOLO)" "$ENV_SETUP; $WAIT_CTRL; $WAIT_ARM; $WAIT_CLOCK; ros2 action send_goal /arm_motion_server/play_motion arm_motion_interfaces/action/PlayMotion \"{motion_name: 'line_follow_init'}\" | tee /tmp/line_follow_init_goal.\$\$.log; grep -q 'status: SUCCEEDED' /tmp/line_follow_init_goal.\$\$.log && ros2 launch line_follow yolo_line_follow.launch.py || echo 'line_follow_init motion did not succeed - yolo_line_follow NOT started. See the goal result above and check the arm_motion_server terminal.' >&2"
  open_term "Camera view"       "$ENV_SETUP; ros2 run rqt_image_view rqt_image_view /yolo_line_follow/debug_image"
  echo "Simulation already running - started the arm server, YOLO line follow, and camera view terminals."
else
  ...
  open_term "Line follow (YOLO)"             "$ENV_SETUP; $WAIT_CTRL; $WAIT_ARM; $WAIT_CLOCK; ros2 action send_goal /arm_motion_server/play_motion arm_motion_interfaces/action/PlayMotion \"{motion_name: 'line_follow_init'}\" | tee /tmp/line_follow_init_goal.\$\$.log; grep -q 'status: SUCCEEDED' /tmp/line_follow_init_goal.\$\$.log && ros2 launch line_follow yolo_line_follow.launch.py || echo 'line_follow_init motion did not succeed - yolo_line_follow NOT started. See the goal result above and check the arm_motion_server terminal.' >&2"
  open_term "Camera view"                    "$ENV_SETUP; ros2 run rqt_image_view rqt_image_view /yolo_line_follow/debug_image"
  echo "Started 4 terminals: Gazebo, Arm motion server, YOLO Line follow, Camera view."
fi
...
echo "  ros2 service call /yolo_line_follow/enable std_srvs/srv/SetBool '{data: false}'"
