#!/usr/bin/env bash
# Put a "Colour Sort (Voice, Vietnamese)" launcher on the Desktop and in the app menu.
#
# Usage:  ./install_voice_color_sort_shortcut.sh [workspace_dir]

set -euo pipefail

WORKSPACE="${1:-${ARM_MOTION_WS:-$HOME/ros2_ws}}"
LAUNCHER="${WORKSPACE}/scripts/run_voice_color_sort.sh"
...
ENTRY="${APPS_DIR}/voice-colour-sort.desktop"
cat >"${ENTRY}" <<DESKTOP
[Desktop Entry]
Type=Application
Version=1.0
Name=Colour Sort (Voice, Vietnamese)
Comment=Control colour sorting, grasping, robot movement, and camera queries using Vietnamese voice commands
Exec=${LAUNCHER}
Path=${WORKSPACE}
Terminal=false
...
DESKTOP_COPY="${DESKTOP_DIR}/voice-colour-sort.desktop"
