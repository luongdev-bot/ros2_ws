#!/usr/bin/env bash
# Launch the Vietnamese voice-controlled grasp-and-delivery demo.
# Gazebo, colour grasping, Nav2 localization, and voice each get a terminal.
# Usage:  run_voice_transport_dietitianl.sh
#
#   Terminal 1: Gazebo (selected saved map + grasp)
#   Terminal 2: Colour perception + grasp executor
#   Terminal 3: Nav2 + RViz
#   Terminal 4: Voice agent (Vietnamese microphone input and Piper speech)
set -u
...
die() {
  ...
  if command -v zenity >/dev/null 2>&1; then
    zenity --error --title="Voice transport delivery" --text="$msg" 2>/dev/null
  elif command -v notify-send >/dev/null 2>&1; then
    notify-send "Voice transport delivery" "$msg" 2>/dev/null
...
WAIT_VOICE_DEPS="echo 'Waiting for Nav2 (/bt_navigator)...'; until ros2 node list 2>/dev/null | grep -qx /bt_navigator; do sleep 2; done; echo 'Waiting for grasp_executor...'; until ros2 node list 2>/dev/null | grep -qx /grasp_executor; do sleep 2; done; sleep 1"
...
open_term "Gazebo ($SELECTED_MAP_NAME + grasp)" "$ENV_SETUP; ros2 launch jetrover_gazebo gazebo_arm.launch.py world:=$SELECTED_WORLD_PATH_Q spawn_x:=$SELECTED_SPAWN_X_Q spawn_y:=$SELECTED_SPAWN_Y_Q spawn_yaw:=$SELECTED_SPAWN_YAW_Q initial_positions_file:=$SELECTED_ARM_POSE_FILE_Q world_name:=$SELECTED_WORLD_NAME_Q"
open_term "Nhận diện màu + tay gắp" "$ENV_SETUP; $WAIT_CTRL; ros2 run arm_perception color_pick --ros-args -p use_sim_time:=true -p start_enabled:=false & color_pick_pid=\$!; ros2 run jetrover_grasp grasp_executor --ros-args -p use_sim_time:=true -p auto_grasp:=false -p mobile_enabled:=true -p 'bin_red:=[-0.135, -0.32, 0.02]' -p 'bin_green:=[-0.055, -0.32, 0.02]' -p 'bin_blue:=[0.025, -0.32, 0.02]' -p 'bin_yellow:=[0.105, -0.32, 0.02]' & grasp_pid=\$!; wait \$color_pick_pid \$grasp_pid"
...
open_term "Voice agent (tiếng Việt)" "$ENV_SETUP; $WAIT_VOICE_DEPS; ros2 launch voice_llm_agent voice_agent.launch.py locations_yaml_path:=$LOCATIONS_Q grasp_executor_node_name:=grasp_executor"

echo "Đã mở 4 terminal:"
echo "  1. Gazebo (map: $SELECTED_MAP_NAME + grasp)"
echo "  2. Nhận diện màu + tay gắp"
echo "  3. Nav2 + RViz"
echo "  4. Voice agent (tiếng Việt)"
echo
...
echo "  arm_transport_function: yêu cầu gắp/đặt theo màu. LƯU Ý: tham số action"
echo "    hiện không phân biệt được pick/place thật; cả hai cùng kích hoạt một chu"
echo "    trình của grasp_executor do giới hạn hệ thống."
