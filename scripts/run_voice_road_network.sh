#!/usr/bin/env bash
# Launch the Vietnamese voice-controlled graph-routed navigation demo. Gazebo,
# Nav2 localization, and the road-network voice agent each get a terminal.
# Usage:  run_voice_road_network.sh
#
#   Terminal 1: Gazebo (selected saved map)
#   Terminal 2: Nav2 + RViz
#   Terminal 3: Road-network voice agent
set -u
...
die() {
  ...
  if command -v zenity >/dev/null 2>&1; then
    zenity --error --title="Voice road network" --text="$msg" 2>/dev/null
  elif command -v notify-send >/dev/null 2>&1; then
    notify-send "Voice road network" "$msg" 2>/dev/null
...
if [ "$SELECTED_MAP_NAME" = "warehouse" ]; then
  ...
  echo "CẢNH BÁO: Đồ thị đường (ROAD_GRAPH) dùng tên địa điểm của map warehouse nên không áp dụng được cho map '$SELECTED_MAP_NAME'; chương trình vẫn tiếp tục chạy." >&2
  LOCATIONS=""
fi
...
open_term "Voice agent (mạng lưới đường)" "$ENV_SETUP; $WAIT_NAV; ros2 run voice_llm_scenarios road_network_tool_executor --ros-args -r __node:=road_network_tool_executor -p camera_topic:=/depth_cam/image -p cmd_vel_topic:=/cmd_vel -p locations_yaml_path:=$LOCATIONS_Q -p ollama_base_url:=http://localhost:11434 -p ollama_model:=qwen2.5vl:3b -p nav_timeout_s:=120.0 & road_executor_pid=\$!; LD_LIBRARY_PATH='$HOME/.local/lib' '$VOICE_PYTHON' -m voice_llm_agent.infrastructure.ros.voice_loop_node --ros-args -p user_utterance_topic:=/road_network_tool_executor/user_utterance -p agent_reply_topic:=/road_network_tool_executor/agent_reply -p whisper_model_size:=small -p whisper_language:=vi -p 'whisper_initial_prompt:=Đây là một đoạn hội thoại tiếng Việt với robot.' -p whisper_device:=cpu -p whisper_compute_type:=int8 -p record_seconds:=5.0 -p silence_rms_threshold:=0.05 -p piper_voice_path:='$PIPER_VOICE' & voice_loop_pid=\$!; wait \$road_executor_pid \$voice_loop_pid"

echo "Đã mở 3 terminal:"
echo "  1. Gazebo (map: $SELECTED_MAP_NAME)"
echo "  2. Nav2 + RViz"
echo "  3. Voice agent (mạng lưới đường)"
echo
...
echo
echo "Khác biệt cốt lõi: move_to_location không gửi một đích duy nhất để robot"
echo "đi thẳng tới đó. Robot lập đường ngắn nhất trên ROAD_GRAPH rồi đi qua NHIỀU"
echo "CHẶNG liên tiếp theo các cạnh của đồ thị đường."
echo
if [ -n "$LOCATIONS" ]; then
  echo "5 địa điểm trong mạng lưới:"
  echo "  Điểm xuất phát"
  echo "  Khu trung tâm"
  echo "  Góc tây nam"
  echo "  Khu vực kệ hàng"
  echo "  Khu tiếp nhận"
  echo
  echo "Các cạnh/quan hệ kề đúng theo ROAD_GRAPH:"
  echo "  Điểm xuất phát -> Khu trung tâm, Góc tây nam"
  echo "  Khu trung tâm -> Điểm xuất phát, Khu vực kệ hàng, Khu tiếp nhận"
  echo "  Góc tây nam -> Điểm xuất phát"
  echo "  Khu vực kệ hàng -> Khu trung tâm"
  echo "  Khu tiếp nhận -> Khu trung tâm"
  echo
else
  echo "Đồ thị đường (ROAD_GRAPH) chỉ định nghĩa cho map warehouse — map '$SELECTED_MAP_NAME' sẽ không định tuyến được qua move_to_location."
fi
echo "Các tool có tác dụng thật: move_to_location (định tuyến theo đồ thị),"
echo "describe_current_view, get_object_box_distance và robot_move_control."
